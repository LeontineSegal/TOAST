...
path_save = f'{path_base}/examples/analyze_one_pixel/outputs' 
...
layers = 1 # total number of layers composing the line-of-sight
constraints_geometry = []  # among {none, 'sandwich'}
number_of_layers_per_clump = layers # number of layer per clump
number_of_clumps = layers // number_of_layers_per_clump

#%% chemistry

# among {'basic', '13co-c18o'}
model_chemistry = 'basic'
...
if model_chemistry == 'basic':
    constraints_abundances = [
        'same_in_all_layers' # respectively to the molecule of reference
    ]
    for key, key_d in colden_ratios_radex.item().items():
        ...
        if key == '13co':
            ...
            # layer 1
            colden_ratios_shifts[key] = [np.round(colden_ratios_ranges[key][0] - key_d, 3)]
            # layer 2 -> last : induce by layer 1, because 'same_in_all_layers' in constraints_abundances
            for layer in range(layers - 1) : 
                colden_ratios_shifts[key].append(np.array([np.nan]))

elif model_chemistry == '13co-c18o':
    constraints_abundances = [
        'same_in_all_layers' # respectively to the molecule of reference
    ]
    ...
    for key, key_d in colden_ratios_radex.item().items():
        ...
        if key == 'c18o':
            ...
            colden_ratios_radex.item()[key] + np.round(np.arange(-0.5, 0.16, 0.05), 2) for layer_idx in range(layers)]
            colden_ratios_shifts[key] = [np.round(colden_ratios_ranges[key][0] - key_d, 3)]
...
#%% kinematics
constraints_kinematics = ['same_C_V_in_all_layers']  # among {'', 'same_C_V_in_all_layers', 'mirror'}
...
# for rw optimization
walkers_per_step = [10000, 1000, 100]
iterations_per_step = [10, 1000, 1000]
