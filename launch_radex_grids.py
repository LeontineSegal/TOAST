...
PARALLELISM, POLL_SIZE = False, 10 # multiprocessing, number of cores to use
...
def mp_launch_radex(
                ...
                log10_N_ref_idx,
                log10_N_ref,
                colden_ratios_name_mol, 
                axis_FWHM, 
                log10_T_kin,
                log10_nH2, 
                colliders): 
        # column density of the considered species
        log10_N = log10_N_ref + colden_ratios_name_mol
        ...
        T_ex, tau, T_r = execute_radex(name_mol,
                f'grid_{name_mol}_{name_line}_{geometry}_N_ref_{log10_N_ref_idx}',
                min_freq,
                max_freq,
        ...
        return T_ex, tau, log10_N_ref_idx
                                   
def mp_launch_radex_wrapped(arg):
    return mp_launch_radex(*arg)  # unpacks args
...
if __name__ == '__main__' : 
        ...
        if sampling_method == 1 : 
                ...
                axis_log10_T_kin = np.linspace(
                        min_log10_T_kin, max_log10_T_kin, num = 1) # start, stop (excluded), samples
                axis_log10_nH2 = np.linspace(
                        min_log10_nH2, max_log10_nH2, num = 1)
                axis_log10_N_ref = np.linspace(
                        min_log10_N_ref, max_log10_N_ref, num = 1)
                axis_FWHM = np.linspace(
                        min_FWHM, max_FWHM, num = 1) 
        ...
        if RUN and OVERWRITE:
                ...
                if PARALLELISM : 
                        ...
                        for name_mol_idx, name_mol  in enumerate(names_mol) : 
                                ...
                                with tqdm.tqdm(total=len(names_line[name_mol_idx])*samples[0]*samples[1]*samples[2], leave = True, position = 0) as pbar:
                                                ...
                                                for log10_T_kin_idx, log10_T_kin in enumerate(axis_log10_T_kin):
                                                        for log10_nH2_idx, log10_nH2 in enumerate(axis_log10_nH2):
                                                                ...
                                                                with Pool(processes=POLL_SIZE) as pool:
                                                                        for res_idx, results_pixel in enumerate(pool.imap_unordered(mp_launch_radex_wrapped, zip_args)):
                                                                                T_ex, tau, log10_N_ref_idx = results_pixel 
                                                                ...
                                                                # clean workspace 
                                                                os.system(f'rm -f grid_{name_mol}_{name_line}_{geometry}_N_ref_*.*')
